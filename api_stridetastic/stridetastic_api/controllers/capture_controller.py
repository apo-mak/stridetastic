...
@api_controller(
    "/captures", tags=["Captures"], permissions=[permissions.IsAuthenticated]
)
class CaptureController:
    ...
    @route.post(
        "/start", response={201: CaptureStartResponse, 400: MessageSchema}, auth=auth
    )
    ...
    @route.post(
        "/{session_id}/stop",
        response={200: CaptureSessionSchema, 404: MessageSchema},
        auth=auth,
    )
    ...
    @route.delete(
        "/{session_id}", response={200: MessageSchema, 404: MessageSchema}, auth=auth
    )
    ...
    def download_capture(self, request, session_id: UUID):
        ...
        response = FileResponse(
            path.open("rb"), content_type=mime_type or "application/octet-stream"
        )
