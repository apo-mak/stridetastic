...
@shared_task(
    name="stridetastic_api.tasks.publisher_tasks.process_periodic_publish_jobs"
)
...
def execute_periodic_publish_job(job_id: int) -> bool:
    ...
    from ..services.service_manager import (  # Local import to avoid circular dependency at load
        ServiceManager,
    )
    ...
    if job.interface_id:
        publisher, base_topic, err = service_manager.resolve_publish_context(
            job.interface_id
        )
        if err:
...
def publish_text_message_task(
    ...
    try:
        ...
        if interface_id is not None:
            ...
            if err:
                logger.error(
                    f"Failed to resolve publisher for interface {interface_id}: {err}"
                )
...
def publish_nodeinfo_task(
    ...
    try:
        ...
        if interface_id is not None:
            ...
            if err:
                logger.error(
                    f"[Publish-Task] Failed to resolve publisher for interface {interface_id}: {err}"
                )
                return {"success": False, "error": err}
...
@shared_task(
    name="stridetastic_api.tasks.publisher_tasks.publish_reachability_probe_task"
)
